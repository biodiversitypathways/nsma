...
::: {.callout-note collapse="true" style="background-color: #f4f4f4; padding: 20px;"}
Advanced users can als use `wildrtrax` with `wt_get_sync(api = "organization_recording_summary", option = "data", organization = 5425)`.
:::
...
# Accumulation curve using vegan::specaccum
...
Figures @fig-spp-activity-trait)–@fig-spp-activity-habitat show seasonal activity patterns by nesting, trait, migratory strategy, and habitat. Any ridgeplots that aren't shown are due to lack of data (i.e. only one or two species detections or guild detections) to plot the data.
