...
We constructed a species accumulation curve using the `vegan` package in R to assess the sampling effort required to capture species richness. The analysis was based on a species-by-site matrix, employing a randomized accumulation method with 100 permutations to ensure robust estimation of species richness across increasing numbers of sampled sites.
